...
const FEATURES = [
  { label: "Amira B.", desc: "Led 12 campaigns · Algiers" },
  { label: "Youcef K.", desc: "150+ volunteer hours · Oran" },
  { label: "Lina M.", desc: "8 local initiatives · Constantine" },
  { label: "Karim D.", desc: "45 cleanify reports · Tizi Ouzou" },
  { label: "Salma H.", desc: "300+ youth engaged · Annaba" },
  { label: "Redouane F.", desc: "Mentored 20 leaders · Blida" },
  { label: "Nadia T.", desc: "5 park cleanups · Bejaia" },
  { label: "Mehdi A.", desc: "Built community tools · Setif" },
  { label: "Fatima Z.", desc: "Food drive organizer · Jijel" },
  { label: "Omar S.", desc: "Street art project · Mostaganem" },
  { label: "Tawfik R.", desc: "Youth forum lead · Batna" },
  { label: "Amine L.", desc: "Tree planting drive · Chlef" },
];

const STATS = [
  { value: "2,400+", label: "Active Members", icon: "people" },
  { value: "180+", label: "Neighborhoods", icon: "location_on" },
  { value: "12K+", label: "Actions Taken", icon: "bolt" },
...
const BubbleNode: React.FC<NodeProps> = ({ data, sizeFactor, isSelected, onSelect, offsetX, offsetY }) => {
  ...
  const visualScale = isSelected ? 5 : (isHovered ? sizeFactor * 1.1 : sizeFactor);
  ...
  return (
    ...
    >
      <div className={`w-full h-full rounded-full overflow-hidden ${data.color} p-1 relative group`}>
        {/* Inner circle */}
        <div className="w-full h-full rounded-full bg-background overflow-hidden relative">
            ...
            <div className={`absolute inset-0 bg-black/70 backdrop-blur-[1px] flex flex-col items-center justify-center p-2 text-center transition-opacity duration-200 ${isSelected || isHovered ? 'opacity-100' : 'opacity-0'}`}>
                <h3 className={`text-white font-bold leading-tight drop-shadow-md ${isSelected ? 'text-4xl mb-4' : 'text-xs sm:text-sm mb-1'}`}>{data.label}</h3>
                <p className={`text-gray-200 leading-tight drop-shadow-sm font-medium ${isSelected ? 'text-lg' : 'text-[8px] sm:text-[9px] line-clamp-3'}`}>{data.description}</p>
            </div>
        </div>
      </div>
    </div>
...
export const CommunityGraph: React.FC = () => {
  ...
  const viewport = { x: dragOffset.x + driftOffset.x, y: dragOffset.y + driftOffset.y };
  ...
  useEffect(() => {
    ...
    const animate = (timestamp: number) => {
      ...
      if (!isDraggingRef.current && selectedNodeIdRef.current === null && timestamp > cooldownUntilRef.current) {
  ...
  const getRepulsionOffset = useCallback((node: NodeData) => {
    if (selectedNodeId === null || selectedNodeId === node.id) return { ox: 0, oy: 0 };
    const selected = nodes.find(n => n.id === selectedNodeId);
    if (!selected) return { ox: 0, oy: 0 };
    ...
    if (dist >= PUSH_THRESHOLD) return { ox: 0, oy: 0 };
      ...
      <div className="max-w-7xl mx-auto md:px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-10 md:mb-14 relative z-20 bg-transparent backdrop-blur-2xl rounded-none p-8  max-w-4xl  mx-auto">
          ...
          <p className="text-base sm:text-lg text-muted-foreground max-w-xl mx-auto">
            Drag to explore real people driving change. Bubbles grow as they approach the center.
          </p>
        </div>
        ...
        >
          ...
          <div 
              ...
              style={{
                  ...
                  transition: isDragging ? 'none' : (selectedNodeId !== null ? 'transform 0.5s cubic-bezier(0.16, 1, 0.3, 1)' : 'none'), 
              }}
          >
              {nodes.map((node) => {
                  ...
                  const edgeFade = selectedNodeId === node.id ? 1 : 
                    Math.max(0, Math.min(1, 1 - (dist - fadeStart) / (fadeEnd - fadeStart)));
                  return (
                      <span key={node.id} style={{ opacity: edgeFade }}>
                          <BubbleNode 
                              data={node}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 mt-12 md:mt-16">
            <Card className="hover:shadow-soft-hover hover:border-primary/30 transition-all duration-300">
                <CardContent className="p-5 sm:p-6 text-center">
                    <span className="material-icons text-4xl text-primary mx-auto mb-2">group</span>
                    <div className="text-2xl sm:text-3xl font-extrabold text-foreground mb-0.5">100+</div>
                    <div className="text-xs sm:text-sm text-muted-foreground font-medium">Active Members</div>
                </CardContent>
            </Card>
            <Card className="hover:shadow-soft-hover hover:border-primary/30 transition-all duration-300">
                <CardContent className="p-5 sm:p-6 text-center">
                    <span className="material-icons text-4xl text-primary mx-auto mb-2">location_on</span>
                    <div className="text-2xl sm:text-3xl font-extrabold text-foreground mb-0.5">4+</div>
                    <div className="text-xs sm:text-sm text-muted-foreground font-medium">Neighborhoods</div>
                </CardContent>
            </Card>
            <Card className="hover:shadow-soft-hover hover:border-primary/30 transition-all duration-300">
                <CardContent className="p-5 sm:p-6 text-center">
                    <span className="material-icons text-4xl text-primary mx-auto mb-2">bolt</span>
                    <div className="text-2xl sm:text-3xl font-extrabold text-foreground mb-0.5">1K+</div>
                    <div className="text-xs sm:text-sm text-muted-foreground font-medium">Actions Taken</div>
                </CardContent>
            </Card>
            <Card className="hover:shadow-soft-hover hover:border-primary/30 transition-all duration-300">
                <CardContent className="p-5 sm:p-6 text-center">
                    <span className="material-icons text-4xl text-primary mx-auto mb-2">star</span>
                    <div className="text-2xl sm:text-3xl font-extrabold text-foreground mb-0.5">4.9</div>
                    <div className="text-xs sm:text-sm text-muted-foreground font-medium">App Rating</div>
                </CardContent>
            </Card>
        </div>
        ...
        <div className="text-center mt-10 md:mt-14">
          <p className="text-sm text-muted-foreground mb-5">Be part of a community reshaping neighborhoods across the country.</p>
          <Button asChild size="lg" className="rounded-full px-8 py-6 text-base font-bold shadow-lg shadow-primary/20 hover:shadow-primary/30 hover:-translate-y-0.5">
          <Link href="/download">
