...
function BreadcrumbLink({
  ...
  return <Comp data-slot="breadcrumb-link" className={cn('hover:text-foreground transition-colors', className)} {...props} dict={dict} />;
}
async function BreadcrumbPage({
  className,
  ...props,
  params
...
function BreadcrumbSeparator({
  ...
  return <li data-slot="breadcrumb-separator" role="presentation" aria-hidden="true" className={cn('[&>svg]:size-3.5', className)} {...props}>
      {children ?? <ChevronRight dict={dict} />}
    </li>;
}
function BreadcrumbEllipsis({
  ...
  return <span data-slot="breadcrumb-ellipsis" role="presentation" aria-hidden="true" className={cn('flex size-9 items-center justify-center', className)} {...props}>
      <MoreHorizontal className="size-4" dict={dict} />
      <span className="sr-only">{dict.BreadcrumbEllipsis.text_1}</span>
    </span>;
}
