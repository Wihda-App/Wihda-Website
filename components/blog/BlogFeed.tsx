...
const CATEGORIES = ['All', 'Impact', 'Community', 'Tech', 'Policy', 'Leadership', 'Story', 'Update'];
...
export function BlogFeed({ initialPosts }: BlogFeedProps) {
  ...
  const filteredPosts = useMemo(() => {
    return initialPosts.filter(post => {
      ...
      return matchesCategory && matchesSearch && post.slug !== featuredPost.slug; // Exclude featured from grid
    });
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center max-w-3xl mx-auto mb-16">
                ...
                <h1 className="text-5xl lg:text-7xl font-sans font-medium tracking-tight text-gray-900 dark:text-white mb-6">
                    Insights for <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary italic">Impact</span>
                </h1>
                <p className="text-xl text-gray-600 dark:text-gray-300 leading-relaxed">
                    Stories, updates, and deep dives from the community building the future of civic engagement.
                </p>
            </div>
                        ...
                        <button
                            ...
                            className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 border ${
                                selectedCategory === category
                                    ? 'bg-gray-900 dark:bg-white text-white dark:text-gray-900 border-transparent shadow-lg transform -translate-y-0.5'
                        ...
                        <div className="flex items-center gap-4">
                             <div className="flex items-center gap-3">
                                ...
                                <div className="text-sm">
                                    <p className="font-bold text-gray-900 dark:text-white">{featuredPost.author.name}</p>
                                    <p className="text-gray-500 dark:text-gray-400">{featuredPost.readTime}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className="lg:col-span-7 order-1 lg:order-2 relative">
                         <Link href={`/blog/${featuredPost.slug}`} className="block overflow-hidden rounded-3xl shadow-2xl">
