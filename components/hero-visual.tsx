...
export const HeroVisual = () => {
  return (
    <div className="relative w-full h-[500px] md:h-[600px] flex items-center justify-center perspective-1000 my-8 md:my-0 scale-90 sm:scale-100">
      ...
      {/* Background Glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[280px] h-[580px] md:w-[320px] md:h-[620px] bg-gradient-to-tr from-primary/20 to-secondary/20 rounded-[3rem] blur-3xl -z-10 animate-pulse-slow"></div>
        ...
        <CardContent className="p-3 md:p-4 flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start">
                    <p className="text-xs md:text-sm font-bold leading-none mb-1 truncate">Amira B.</p>
                    <Badge variant="outline" className="text-[9px] h-4 px-1 border-primary/20 text-primary">New</Badge>
                </div>
                <p className="text-[10px] md:text-xs text-muted-foreground truncate">Joined <span className="text-foreground font-medium">Algiers Community</span></p>
                <div className="flex items-center gap-1 mt-1">
                    <MapPin className="w-3 h-3 text-muted-foreground" />
                    <span className="text-[9px] text-muted-foreground">Hydra, Algiers</span>
                </div>
            </div>
        </CardContent>
      </Card>
      ...
      <Card className="absolute bottom-[15%] -right-[8%] md:bottom-32 md:-right-12 w-52 md:w-60 shadow-xl border-secondary/20 animate-float-medium z-40 bg-background/95 backdrop-blur hover:scale-105 transition-transform duration-300">
        <CardContent className="p-3 md:p-4">
            <div className="flex justify-between items-start mb-2">
                <Badge variant="secondary" className="text-[9px] md:text-[10px] h-5 px-1.5 bg-secondary/10 text-secondary hover:bg-secondary/20 border border-secondary/20">
                    <Calendar className="w-3 h-3 mr-1" />
                    Event
                </Badge>
                <span className="text-[9px] md:text-[10px] text-muted-foreground bg-muted px-1.5 py-0.5 rounded-full">2h ago</span>
            </div>
            <h4 className="text-xs md:text-sm font-bold mb-1 line-clamp-1">Beach Cleanup Drive</h4>
            <div className="flex items-center justify-between mt-2">
                <div className="flex items-center gap-1.5">
                    <div className="flex -space-x-2">
                         <Avatar className="w-5 h-5 border-2 border-background"><AvatarFallback className="text-[8px] bg-primary/10">A</AvatarFallback></Avatar>
                         <Avatar className="w-5 h-5 border-2 border-background"><AvatarFallback className="text-[8px] bg-secondary/10">B</AvatarFallback></Avatar>
                         <Avatar className="w-5 h-5 border-2 border-background"><AvatarFallback className="text-[8px] bg-muted">C</AvatarFallback></Avatar>
                    </div>
                    <span className="text-[9px] md:text-[10px] text-muted-foreground font-medium">+12 going</span>
                </div>
       ...
       <Card className="absolute bottom-[40%] -left-[5%] md:bottom-24 md:-left-8 w-56 md:w-64 shadow-xl border-yellow-500/20 animate-float-fast z-50 bg-background/95 backdrop-blur hover:scale-105 transition-transform duration-300">
        <CardContent className="p-3 md:p-4 flex gap-3 text-left">
            ...
            <div>
                <p className="text-xs md:text-sm font-bold text-yellow-600 mb-0.5">Reward Unlocked!</p>
                <p className="text-[10px] md:text-xs text-foreground/80 leading-tight">You earned <span className="font-bold text-yellow-600">50 points</span> for verifying a report.</p>
                <div className="w-full bg-muted h-1 mt-2 rounded-full overflow-hidden">
                    <div className="bg-yellow-500 h-full w-3/4 rounded-full"></div>
                </div>
                <p className="text-[9px] text-muted-foreground mt-1 text-right">75% to next level</p>
            </div>
        </CardContent>
      </Card>
